...
class Bubble:
    ...
    # Move bubbles around the Screen
    def move(self):
    ...
    # Show/Draw the bubble 
    def show(self):
        pygame.draw.ellipse(display, self.color, (self.x, self.y, self.a, self.b))

    # Check if bubbles is bursted
    def burst(self):
        global score
        pos = pygame.mouse.get_pos()

        if onBubble(self.x, self.y, self.a, self.b, pos):
            score += 1
            self.reset()
...
bubble = []
noBubble = 10 
for i in range(noBubble):
    obj = Bubble(random.choice([1, 1, 2, 2, 2, 2, 3, 3, 3, 4]))
    bubble.append(obj)

def onBubble(x, y, a, b, pos):
...
def pointer():
    ...
    color = lightGreen
    for i in range(noBubble):
        if onBubble(bubble[i].x, bubble[i].y, bubble[i].a, bubble[i].b, pos):
            color = red
...
def lowerPlatform():
    pygame.draw.rect(display, green, (0, height - lowerBound, width, lowerBound))

def showScore():
    scoreText = font.render("Bubbles Bursted : " + str(score), True, white)
